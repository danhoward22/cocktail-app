...
A React app for browsing classic cocktail recipes: search by name or by ingredient, then pull up a recipe card with measures, garnish, and notes.
...
1. **Wire up "Add New Cocktail" and "Add New Ingredient"** — Build out real forms to save custom cocktails and ingredients to `localStorage` as a proof of concept.
...
3. **Replace the in-memory data source with a real API** — `fetchCocktailList` and `fetchCocktail` are structured to make this a drop-in swap; add loading/error states where needed.
4. **Create a bulk upload page** — Build out a page that parses a csv file of cocktail recipes, and uploads the recipes to the database, conditionally creating the unknown 
5. **Accessibility pass** — audit the custom `UnitSelect` and search toggle for keyboard navigation and screen-reader labeling now that there's more interactive UI per row.
6. **Tests** — no test setup exists yet; start with unit tests for `unitUtils.js` (conversion math and fraction rounding are the highest-value, easiest-to-break logic in the app).
